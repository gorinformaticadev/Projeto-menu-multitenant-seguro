...
const diagnosticsCardClassName =
  "rounded-[2rem] border border-slate-200/80 bg-white/90 shadow-sm shadow-slate-950/5 dark:border-slate-800/80 dark:bg-slate-950/45 dark:shadow-black/25";
const diagnosticsPanelClassName =
  "rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3 dark:border-slate-800 dark:bg-slate-950/55";
const diagnosticsPanelStrongClassName =
  "rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3 text-sm text-slate-700 dark:border-slate-800 dark:bg-slate-950/55 dark:text-slate-200";
          ...
          <div className="flex items-center gap-2">
            ...
            {href ? (
              <Button
                ...
                variant="outline"
                size="sm"
                className="dark:border-slate-700 dark:bg-slate-950/50 dark:text-slate-200 dark:hover:bg-slate-900"
              >
      ...
      <div className="space-y-6 p-4 md:p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-600 dark:border-slate-800/90 dark:bg-slate-950/60 dark:text-slate-300">
            ...
            {Array.from({ length: 4 }).map((_, index) => (
              <Card key={index} className={diagnosticsCardClassName}>
                <CardContent className="space-y-3 p-6">
                  <div className="h-4 w-32 animate-pulse rounded-full bg-slate-200 dark:bg-slate-800/90" />
                  <div className="h-8 w-40 animate-pulse rounded-full bg-slate-100 dark:bg-slate-900/90" />
                  <div className="h-3 w-full animate-pulse rounded-full bg-slate-100 dark:bg-slate-900/90" />
                </CardContent>
              </Card>
            ))}
          </div>
        ...
        {data ? (
          <>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <Card className={diagnosticsCardClassName}><CardContent className="space-y-3 p-6"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400"><Activity className="h-4 w-4" />Estado geral</div><div className={cn("text-2xl font-semibold", overall.accentClassName)}>{data.overall.label}</div><p className="text-sm text-muted-foreground">{data.overall.summary}</p></CardContent></Card>
              <Card className={diagnosticsCardClassName}><CardContent className="space-y-3 p-6"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400"><Wrench className="h-4 w-4" />Maintenance</div><div className="text-2xl font-semibold text-slate-900 dark:text-slate-100">{data.overall.maintenanceActive ? "Ativo" : "Inativo"}</div><p className="text-sm text-muted-foreground">Versao {data.overall.version || "Sem leitura"} - Uptime {data.overall.uptimeHuman || "Sem leitura"}</p></CardContent></Card>
              <Card className={diagnosticsCardClassName}><CardContent className="space-y-3 p-6"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400"><Clock3 className="h-4 w-4" />Tarefas</div><div className="text-2xl font-semibold text-slate-900 dark:text-slate-100">{scheduler ? `${Array.isArray(scheduler.problematic) ? scheduler.problematic.length : 0} com atencao` : "Leitura parcial"}</div><p className="text-sm text-muted-foreground">{scheduler ? `${scheduler.enabled} ativas de ${scheduler.total}` : data.scheduler.message}</p></CardContent></Card>
              <Card className={diagnosticsCardClassName}><CardContent className="space-y-3 p-6"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400"><BellRing className="h-4 w-4" />Alertas</div><div className="text-2xl font-semibold text-slate-900 dark:text-slate-100">{alerts ? String(alerts.recentCount || 0) : "Parcial"}</div><p className="text-sm text-muted-foreground">{alerts ? `${alerts.criticalCount} criticos na janela recente` : data.alerts.message}</p></CardContent></Card>
            </div>
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              ...
              <SectionCard title="Auditoria e logs" description="Reaproveita a pagina /logs existente e mostra a cobertura real dos dados." section={data.logs} href={data.links.logs} actionLabel="Abrir logs">
                <div className="space-y-4"><div className={diagnosticsPanelClassName}><p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{String(logs?.summary || "Sem resumo")}</p><p className="mt-1 text-xs text-muted-foreground">Escopo atual: {String(logs?.pageKind || "auditoria")}</p></div><div className="flex flex-wrap gap-2">{Array.isArray(logs?.coverage) ? logs.coverage.map((item: string) => <Badge key={item} variant="outline" className="dark:border-slate-700 dark:bg-slate-950/50 dark:text-slate-200">{item}</Badge>) : null}</div>{Array.isArray(logs?.recentTechnicalIssues) && logs.recentTechnicalIssues.length > 0 ? logs.recentTechnicalIssues.map((item: Record<string, any>) => <div key={String(item.id)} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-950/55"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{String(item.title)}</p><Badge variant="outline" className="dark:border-slate-700 dark:bg-slate-950/50 dark:text-slate-200">{String(item.origin)}</Badge></div><p className="mt-1 text-sm text-muted-foreground">{String(item.detail || "Sem detalhe adicional")}</p><div className="mt-2 text-xs text-muted-foreground">{formatDate(item.occurredAt)}</div></div>) : <DashboardSurfaceState title="Sem falhas tecnicas recentes" description="Nao foram identificadas falhas tecnicas recentes nas bases reaproveitadas." tone="neutral" />}</div>
              </SectionCard>
              <SectionCard title="Eventos criticos de auditoria" description="Ultimos eventos criticos relevantes ja persistidos." section={data.audit} href={data.links.audit} actionLabel="Abrir auditoria">
                <div className="space-y-2">{Array.isArray(audit?.recent) && audit.recent.length > 0 ? audit.recent.map((item: Record<string, any>) => <div key={String(item.id)} className={diagnosticsPanelClassName}><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{String(item.actionLabel || item.action)}</p><Badge variant="outline" className="dark:border-slate-700 dark:bg-slate-950/50 dark:text-slate-200">{formatLabel(item.severity)}</Badge></div><p className="mt-1 text-sm text-muted-foreground">{String(item.message || item.action)}</p><div className="mt-2 text-xs text-muted-foreground">{formatDate(item.createdAt)}</div></div>) : <DashboardSurfaceState title="Sem eventos criticos" description="A auditoria nao registrou eventos criticos recentes nesta visao." tone="neutral" />}</div>
              </SectionCard>
            </div>
          </>
        ) : null}
