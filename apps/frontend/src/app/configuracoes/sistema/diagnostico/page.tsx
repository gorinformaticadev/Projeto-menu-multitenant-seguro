...
function SectionCard({
  ...
  return (
    <Card className="rounded-[2rem] border-slate-200/80 shadow-sm">
      <CardHeader className="space-y-3 pb-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {href ? <Button asChild variant="outline" size="sm"><Link href={href}>{actionLabel || "Abrir"}</Link></Button> : null}
          </div>
        </div>
      </CardHeader>
      ...
      <div className="space-y-6 p-4 md:p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-600"><FileSearch className="h-3.5 w-3.5" />Diagnostico operacional</div>
            <h1 className="text-3xl font-bold tracking-tight">Visao unificada do sistema</h1>
            <p className="max-w-3xl text-sm text-muted-foreground">Reaproveita dashboard, cron runtime, updates, backup, restore, auditoria, notificacoes e a funcionalidade de logs ja existente.</p>
          </div>
        ...
        {loading && !data ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{Array.from({ length: 4 }).map((_, index) => <Card key={index} className="rounded-[2rem] border-slate-200/80 shadow-sm"><CardContent className="space-y-3 p-6"><div className="h-4 w-32 animate-pulse rounded-full bg-slate-200" /><div className="h-8 w-40 animate-pulse rounded-full bg-slate-100" /><div className="h-3 w-full animate-pulse rounded-full bg-slate-100" /></CardContent></Card>)}</div> : null}

        {data ? (
          <>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <Card className="rounded-[2rem] border-slate-200/80 shadow-sm"><CardContent className="space-y-3 p-6"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500"><Activity className="h-4 w-4" />Estado geral</div><div className={cn("text-2xl font-semibold", overall.accentClassName)}>{data.overall.label}</div><p className="text-sm text-muted-foreground">{data.overall.summary}</p></CardContent></Card>
              <Card className="rounded-[2rem] border-slate-200/80 shadow-sm"><CardContent className="space-y-3 p-6"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500"><Wrench className="h-4 w-4" />Maintenance</div><div className="text-2xl font-semibold">{data.overall.maintenanceActive ? "Ativo" : "Inativo"}</div><p className="text-sm text-muted-foreground">Versao {data.overall.version || "Sem leitura"} - Uptime {data.overall.uptimeHuman || "Sem leitura"}</p></CardContent></Card>
              <Card className="rounded-[2rem] border-slate-200/80 shadow-sm"><CardContent className="space-y-3 p-6"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500"><Clock3 className="h-4 w-4" />Tarefas</div><div className="text-2xl font-semibold">{scheduler ? `${Array.isArray(scheduler.problematic) ? scheduler.problematic.length : 0} com atencao` : "Leitura parcial"}</div><p className="text-sm text-muted-foreground">{scheduler ? `${scheduler.enabled} ativas de ${scheduler.total}` : data.scheduler.message}</p></CardContent></Card>
              <Card className="rounded-[2rem] border-slate-200/80 shadow-sm"><CardContent className="space-y-3 p-6"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500"><BellRing className="h-4 w-4" />Alertas</div><div className="text-2xl font-semibold">{alerts ? String(alerts.recentCount || 0) : "Parcial"}</div><p className="text-sm text-muted-foreground">{alerts ? `${alerts.criticalCount} criticos na janela recente` : data.alerts.message}</p></CardContent></Card>
            </div>

            {data.overall.reasons.length > 0 ? <Card className="rounded-[2rem] border-slate-200/80 shadow-sm"><CardHeader><CardTitle className="text-lg">Leitura rapida do estado atual</CardTitle><CardDescription>Motivos que sustentam a classificacao atual do sistema.</CardDescription></CardHeader><CardContent><div className="grid gap-2 md:grid-cols-2">{data.overall.reasons.map((reason) => <div key={reason} className="rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3 text-sm text-slate-700">{reason}</div>)}</div></CardContent></Card> : null}
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              ...
              <SectionCard title="Scheduler e tarefas" description="Runtime real das tarefas, com falhas, atrasos e travamentos." section={data.scheduler} href={data.links.cron} actionLabel="Abrir tarefas">
                <div className="space-y-4"><div className="grid gap-3 md:grid-cols-4"><Metric label="Ativas" value={String(scheduler?.enabled || 0)} /><Metric label="Com falha" value={String(scheduler?.failed || 0)} /><Metric label="Atrasadas" value={String(scheduler?.stale || 0)} /><Metric label="Travadas" value={String(scheduler?.stuck || 0)} /></div>{Array.isArray(scheduler?.problematic) && scheduler.problematic.length > 0 ? scheduler.problematic.map((item: Record<string, any>) => <div key={String(item.key)} className="rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-semibold text-slate-900">{String(item.name)}</p><Badge variant="outline">{formatLabel(item.type)}</Badge></div><p className="mt-1 text-xs text-muted-foreground">{String(item.summary)}</p><div className="mt-2 grid gap-2 text-xs text-muted-foreground md:grid-cols-2"><span>Proximo esperado: {formatDate(item.nextExpectedRunAt)}</span><span>Ultimo sucesso: {formatDate(item.lastSucceededAt)}</span></div></div>) : <DashboardSurfaceState title="Sem tarefas problematicas" description="O runtime nao reportou atrasos ou falhas relevantes." tone="neutral" />}</div>
              </SectionCard>
              ...
              <SectionCard title="Alertas operacionais" description="Ultimos alertas automaticos, sem criar uma nova inbox." section={data.alerts} href={alerts?.href || null} actionLabel="Abrir notificacoes">
                <div className="space-y-4"><div className="grid gap-3 md:grid-cols-3"><Metric label="Alertas recentes" value={String(alerts?.recentCount || 0)} /><Metric label="Criticos" value={String(alerts?.criticalCount || 0)} /><Metric label="Inbox dedicada" value={alerts?.inboxAvailable ? "Disponivel" : "Nao disponivel"} /></div>{Array.isArray(alerts?.recent) && alerts.recent.length > 0 ? alerts.recent.map((item: Record<string, any>) => <div key={String(item.id)} className="rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-semibold text-slate-900">{String(item.title)}</p><Badge variant="outline">{formatLabel(item.severity)}</Badge></div><p className="mt-1 text-sm text-muted-foreground">{String(item.body)}</p><div className="mt-2 text-xs text-muted-foreground">{formatDate(item.createdAt)}</div></div>) : <DashboardSurfaceState title="Sem alertas recentes" description="Nenhum alerta operacional foi emitido na janela atual." tone="neutral" />}</div>
              </SectionCard>
              <SectionCard title="Auditoria e logs" description="Reaproveita a pagina /logs existente e mostra a cobertura real dos dados." section={data.logs} href={data.links.logs} actionLabel="Abrir logs">
                <div className="space-y-4"><div className="rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3"><p className="text-sm font-semibold text-slate-900">{String(logs?.summary || "Sem resumo")}</p><p className="mt-1 text-xs text-muted-foreground">Escopo atual: {String(logs?.pageKind || "auditoria")}</p></div><div className="flex flex-wrap gap-2">{Array.isArray(logs?.coverage) ? logs.coverage.map((item: string) => <Badge key={item} variant="outline">{item}</Badge>) : null}</div>{Array.isArray(logs?.recentTechnicalIssues) && logs.recentTechnicalIssues.length > 0 ? logs.recentTechnicalIssues.map((item: Record<string, any>) => <div key={String(item.id)} className="rounded-2xl border border-slate-200 bg-white px-4 py-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-semibold text-slate-900">{String(item.title)}</p><Badge variant="outline">{String(item.origin)}</Badge></div><p className="mt-1 text-sm text-muted-foreground">{String(item.detail || "Sem detalhe adicional")}</p><div className="mt-2 text-xs text-muted-foreground">{formatDate(item.occurredAt)}</div></div>) : <DashboardSurfaceState title="Sem falhas tecnicas recentes" description="Nao foram identificadas falhas tecnicas recentes nas bases reaproveitadas." tone="neutral" />}</div>
              </SectionCard>
              <SectionCard title="Eventos criticos de auditoria" description="Ultimos eventos criticos relevantes ja persistidos." section={data.audit} href={data.links.audit} actionLabel="Abrir auditoria">
                <div className="space-y-2">{Array.isArray(audit?.recent) && audit.recent.length > 0 ? audit.recent.map((item: Record<string, any>) => <div key={String(item.id)} className="rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-semibold text-slate-900">{String(item.actionLabel || item.action)}</p><Badge variant="outline">{formatLabel(item.severity)}</Badge></div><p className="mt-1 text-sm text-muted-foreground">{String(item.message || item.action)}</p><div className="mt-2 text-xs text-muted-foreground">{formatDate(item.createdAt)}</div></div>) : <DashboardSurfaceState title="Sem eventos criticos" description="A auditoria nao registrou eventos criticos recentes nesta visao." tone="neutral" />}</div>
              </SectionCard>
            </div>
          </>
        ) : null}
