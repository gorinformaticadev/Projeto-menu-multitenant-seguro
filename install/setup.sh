...
TS_NODE_ESM_BIN="/app/apps/backend/node_modules/.bin/ts-node-esm"
docker compose run --rm -e DATABASE_URL="$DB_URL_VAL" -w /app backend "$PRISMA_BIN" migrate deploy --schema /app/prisma/schema.prisma
docker compose run --rm -e DATABASE_URL="$DB_URL_VAL" -w /app/apps/backend backend "$TS_NODE_ESM_BIN" prisma/seed.ts
