...
run_install() {
    ...
    # Corrigir possíveis marcadores não substituídos no template
    sed -i "s/__DB_NAME__/$db_name/g" "$ENV_PRODUCTION"
    sed -i "s/__DB_USER__/$db_user/g" "$ENV_PRODUCTION"
    sed -i "s/__DB_PASSWORD__/$db_pass/g" "$ENV_PRODUCTION"
