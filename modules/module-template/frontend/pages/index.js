 ...
 * 
 * Template para criação de páginas de módulos independentes
 * JavaScript puro, sem dependências externas
 */

function ModuleTemplatePage() {
  // Inicializar o ModuleCore se disponível
  let core = null;
  let bridge = null;
  
  console.log('🔍 [Template] Verificando disponibilidade do sistema...');
  
  if (typeof window !== 'undefined' && window.ModuleCore) {
    core = window.ModuleCore;
    console.log('✅ [Template] ModuleCore encontrado');
    
    // Tentar inicializar com o bridge do sistema
    if (typeof ModuleBridge !== 'undefined') {
      core.init(ModuleBridge);
      bridge = core.getBridge();
      console.log('🔧 [Template] Módulo inicializado com bridge do sistema');
    } else {
      bridge = core.getBridge(); // Usará fallback
      console.log('🔧 [Template] Módulo inicializado com bridge de fallback');
    }
  } else {
    console.log('⚠️ [Template] ModuleCore não encontrado, usando modo básico');
  }
  ...
  const render = () => {
    ...
    const title = createElement('h1', { className: 'text-2xl font-bold text-gray-900' }, 'Module Template');
    const subtitle = createElement('p', { className: 'text-gray-600' }, 'Template para criação de módulos independentes');
    ...
    const steps = [
      '1. Copie a pasta module-template para um novo nome (ex: module-meumodulo)',
    ...
    const statusMessage = bridge && core && core.isReady() ? 
      'Sistema Integrado - Template funcionando com bridge do sistema' :
      'Modo Independente - Template funcionando sem dependências externas';
    ...
    if (!bridge) {
      ...
      const infoTitle = createElement('h3', { className: 'text-sm font-medium text-purple-800' }, 'Módulo Template');
...
if (typeof window !== 'undefined') {
  window.ModuleTemplatePage = ModuleTemplatePage;
}