 ...
 * 
 * Template para páginas de configurações de módulos
 * JavaScript puro, sem dependências externas
 */

function ModuleTemplateSettingsPage() {
  ...
  const render = () => {
    ...
    const title = createElement('h1', { className: 'text-2xl font-bold text-gray-900' }, 'Configurações do Template');
    const subtitle = createElement('p', { className: 'text-gray-600' }, 'Template para páginas de configurações');
    ...
    const templateBadge = createElement('span', { 
    ...
    const restoreButton = createElement('button', { 
      ...
      onclick: () => {
        ...
        if (nameInput) nameInput.value = 'Module Template';
    ...
    const templateCard = createElement('div', { className: 'bg-white shadow overflow-hidden sm:rounded-lg' });
    const templateHeader = createElement('div', { className: 'px-4 py-5 sm:p-6' });
    const templateTitle = createElement('h3', { className: 'text-lg leading-6 font-medium text-gray-900 mb-4' }, '📋 Sobre este Template');
    const templateDesc = createElement('p', { className: 'text-sm text-gray-600 mb-6' }, 'Informações sobre como usar este template');
    
    const templateInfo = createElement('div', { className: 'space-y-3' });
    
    const templatePoints = [
      'Este é um template para páginas de configurações',
    ...
    templatePoints.forEach(point => {
      ...
      templateInfo.appendChild(pointEl);
    });
    
    templateHeader.appendChild(templateTitle);
    templateHeader.appendChild(templateDesc);
    ...
    const infoTitle = createElement('h3', { className: 'text-sm font-medium text-purple-800' }, 'Template de Configurações');
...
if (typeof window !== 'undefined') {
  window.ModuleTemplateSettingsPage = ModuleTemplateSettingsPage;
}