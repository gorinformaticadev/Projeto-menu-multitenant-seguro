-- ═══════════════════════════════════════════════════════════════════════════
-- MIGRATION: Criação das tabelas do módulo sistema
-- Versão: 1.0.0
-- Data: 2025-12-17
-- ═══════════════════════════════════════════════════════════════════════════

-- Tabela de configurações do sistema
-- Armazena configurações específicas do módulo por tenant
CREATE TABLE IF NOT EXISTS sistema_configs (
    -- Identificador único da configuração
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    
    -- ID do tenant ao qual esta configuração pertence
    -- Garante isolamento de dados entre tenants
    tenant_id TEXT NOT NULL,
    
    -- Chave da configuração (ex: 'module_enabled', 'version')
    key VARCHAR(255) NOT NULL,
    
    -- Valor da configuração (armazenado como texto)
    value TEXT,
    
    -- Timestamps de criação e atualização
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,

    -- Foreign Key para tabela tenants
    -- Garante integridade referencial e exclusão em cascata
    CONSTRAINT fk_sistema_configs_tenant FOREIGN KEY (tenant_id)
        REFERENCES tenants(id) ON DELETE CASCADE
);

-- Índices para melhorar a performance das consultas
CREATE INDEX IF NOT EXISTS idx_sistema_configs_tenant_id ON sistema_configs(tenant_id);
CREATE INDEX IF NOT EXISTS idx_sistema_configs_key ON sistema_configs(key);